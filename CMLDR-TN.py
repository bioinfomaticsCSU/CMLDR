...
def optimize(model, sampler, train, valid, test):
    ...
    train_users = list(set(train.nonzero()[0]))
    
    stop_num =10
    t_stop_num = 0
    stop_threshold = 0.005
    pre_recall = 0
    k_Mat = [1, 5, 10, 20, 30, 40, 50]
    r_recalls = np.zeros([1, 7])
    r_precisions = np.zeros([1, 7])
    while True:
        ...
        if t_stop_num > stop_num:
            ...
            r_aupr = test_recall.eval_roc(sess, test_users, test)
            # compute recall in chunks to utilize speedup provided by Tensorflow
...
if __name__ == '__main__':
    ...
    dense_features = None
    Ks_test_recalls = np.zeros([10, 7])
    Ks_test_precisions = np.zeros([10, 7])
